...
import { Search, FileText, Award, Zap, Tag, History, Mic } from "lucide-react";
...
const VoiceLibrary = () => {
  ...
  const { data: allLessons, loading: lessonsLoading } = useApi(fetchLessons, []);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8 pb-6 border-b border-white/[0.07]">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight mb-1.5">
            ...
            <span className="text-[#f5a623]">{t('voiceLibrary.titleSuffix')}</span>
          </h1>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <aside className="lg:w-56 shrink-0">
          ...
          <div className="flex flex-col gap-1">
            <button
              ...
              className={`flex items-center justify-between px-3 py-2 rounded-lg text-[13px] font-medium transition-colors ${
                activeCategory === "All"
                  ? 'bg-[#f5a623]/[0.1] text-[#f5a623] border border-[#f5a623]/20'
                  : 'text-zinc-400 hover:text-white hover:bg-white/[0.05]'
              }`}
            >
              <span>{t('voiceLibrary.allScripts')}</span>
              <span className={`text-[11px] px-1.5 py-0.5 rounded ${activeCategory === "All" ? 'bg-[#f5a623]/20 text-[#f5a623]' : 'bg-white/[0.06] text-zinc-600'}`}>
                {lessons.length}
              </span>
            </button>
            {categories.map(cat => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`text-[11px] px-1.5 py-0.5 rounded ${active ? 'bg-[#f5a623]/20 text-[#f5a623]' : 'bg-white/[0.06] text-zinc-600'}`}>
                    {count}
                  </span>
                </button>
              );
          ...
          <div className="mt-6 p-4 bg-[#111113] border border-white/[0.07] rounded-xl">
            <Zap size={18} className="text-[#f5a623] mb-2.5" />
            ...
            <div className="flex gap-2 flex-wrap">
              {[
                ...
                <select
                  ...
                  className="bg-[#111113] border border-white/[0.07] text-zinc-300 text-[12px] py-2 px-3 rounded-xl focus:outline-none focus:border-[#f5a623]/30 cursor-pointer"
                >
            ...
            <div className="space-y-2">
              {currentItems.map((lesson) => {
                ...
                return (
                  <div
                    key={lesson.id}
                    className="flex items-center gap-4 p-4 bg-[#111113] border border-white/[0.07] rounded-xl hover:border-white/[0.12] hover:bg-[#1a1a1e] transition-colors group"
                  >
                    {/* Icon */}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        <h3 className="text-[14px] font-semibold text-white truncate group-hover:text-white">
                          {lesson.title}
                        </h3>
                        <span className={`shrink-0 text-[10px] font-medium px-1.5 py-0.5 rounded border ${difficultyStyle(lesson.difficulty)}`}>
                          {lesson.difficulty}
                        </span>
                        {stats.total > 0 && (
                          <span className="shrink-0 text-[10px] font-medium px-1.5 py-0.5 rounded bg-emerald-500/[0.08] text-emerald-400 border border-emerald-500/20">
                      ...
                      <div className="flex items-center gap-3 text-[12px] text-zinc-600">
                        <span className="flex items-center gap-1">
                          <Tag size={10} className="text-[#f5a623]" />
                    ...
                    <button
                      ...
                      className="shrink-0 flex items-center gap-1.5 px-4 py-2 rounded-lg bg-[#f5a623] text-black text-[12px] font-semibold hover:bg-[#e09520] transition-colors"
                    >
                      <Zap size={13} />
                      {t('common.start')}
                    </button>
                ...
                <button
                  ...
                  className={`w-8 h-8 rounded-lg text-[12px] font-medium transition-colors ${
                    currentPage === p
                      ...
                      : 'bg-[#111113] border border-white/[0.07] text-zinc-400 hover:bg-[#1a1a1e]'
                  }`}
                >
                  {p}
                </button>
