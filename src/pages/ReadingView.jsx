...
const ReadingView = () => {
  ...
  if (!guide) return <div className="text-white text-center py-40">{t('readingView.guideNotFound')}</div>;
              ...
              {curriculum.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-md transition-colors border flex items-center gap-3 ${isActive
                      ? 'bg-[#f5a623]/[0.08] border-[#f5a623]/20 text-[#f5a623]'
                      : 'border-transparent text-zinc-500 hover:bg-white/[0.04] hover:text-zinc-300'
                      }`}
                  >
                    <div className={`w-7 h-7 rounded-md flex items-center justify-center shrink-0 ${isActive ? 'bg-[#f5a623] text-black' : 'bg-[#111113] text-zinc-500'
                      }`}>
        ...
        <div className="flex-1 overflow-y-auto h-[calc(100vh-6rem)]">
          ...
          <div className="max-w-3xl mx-auto px-6 lg:px-10 py-14">
            {courseId && !mId && (
              <div className="mb-8">
                <Breadcrumb items={[
                  { label: t('readingView.courses'), href: '/m/courses' },
                  { label: course?.title || t('readingView.courseDetail'), href: `/m/courses/${courseId}` },
                  { label: guide.title }
                ]} />
              </div>
            )}
            ...
            <div className="mt-4 space-y-6 mb-12">
              ...
              <div className="flex items-center justify-between py-5 border-y border-white/[0.06]">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-md bg-[#f5a623] flex items-center justify-center text-black font-bold">
                    {guide.author?.charAt(0) || 'A'}
                  </div>
                  <div>
                    <p className="text-[13px] font-semibold text-white">{guide.author || t('readingView.mchubElite')}</p>
                    <p className="text-[11px] text-zinc-500">{t('readingView.industryMentor')}</p>
                  </div>
                </div>
                <Button
                  onClick={() => setIsNotesOpen(true)}
                  className="w-9 h-9 rounded-md bg-[#111113] border border-white/[0.07] flex items-center justify-center text-[#f5a623] hover:text-white transition-colors relative"
                >
                ...
                <Button className="w-9 h-9 rounded-md bg-[#111113] border border-white/[0.07] flex items-center justify-center text-zinc-500 hover:text-white transition-colors">
                  <Share2 size={15} />
                </Button>
              </div>
            </div>
            ...
            {/* Footer CTA */}
            <div className="bg-[#111113] border border-white/[0.07] rounded-md p-6 flex flex-col md:flex-row items-center justify-between gap-6">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-md bg-emerald-500 flex items-center justify-center text-black">
                  <CheckCircle2 size={24} />
                </div>
                <div>
                  <h3 className="text-[15px] font-semibold text-white">{t('readingView.markAsCompleted')}</h3>
              ...
              <Button
                onClick={async () => {
                  ...
                  if (courseId) {
                    ...
                    celebrate(t('readingView.celebrateMsg'));
                    navigate(`/m/courses/${courseId}`);
                  } else {
                    navigate(-1);
                  }
                }}
                className="flex items-center gap-2 px-6 py-2.5 bg-[#f5a623] text-black rounded-md text-[13px] font-semibold hover:bg-[#e09520] transition-colors shrink-0 h-auto"
              >
