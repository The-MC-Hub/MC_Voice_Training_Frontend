      ...
      <div className="bg-[#111113] border border-white/[0.07] rounded-2xl p-10 max-w-md w-full text-center space-y-6">
        ...
        {verified === null ? (
          <div className="flex flex-col items-center gap-4 py-8">
            ...
            <p className="text-[12px] text-zinc-500 uppercase tracking-widest">{t('payment.verifyingPayment')}</p>
          </div>
        ) : actualSuccess ? (
          <>
            ...
            <h2 className="text-xl font-bold text-white">{t('payment.paymentConfirmed')}</h2>
            <p className="text-[13px] text-zinc-400 leading-relaxed">
              {t('payment.premiumAccountActivatedPrefix')} <span className="text-[#f5a623] font-semibold">Premium</span> {t('payment.premiumAccountActivatedSuffix')}
            </p>
          </>
        ) : (
          <>
            ...
            <h2 className="text-xl font-bold text-white">
              {isSuccess ? t('payment.paymentPending') : t('payment.paymentCancelled')}
            </h2>
            <p className="text-[13px] text-zinc-400 leading-relaxed">
              {isSuccess
                ? t('payment.transactionNotConfirmed')
