        ...
        >
              ...
              {[
                ...
                <div key={m.label} className="group/m flex items-center gap-3 py-2 border-b border-white/[0.04] last:border-0">
                  ...
                  <div className="relative cursor-help shrink-0">
                    <Info size={11} className="text-zinc-500 group-hover/m:text-zinc-500 transition-colors" />
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="p-3 rounded-md bg-[#09090b] border border-white/[0.05]">
                      <div className="flex items-center gap-1 mb-1">
                        ...
                        <div className="relative group/tt cursor-help"><Info size={9} className="text-zinc-500" /><div className="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-52 rounded-md bg-[#1a1a1e] border border-white/[0.08] p-3 text-[11px] text-zinc-400 leading-relaxed opacity-0 group-hover/tt:opacity-100 transition-opacity z-50 shadow-xl text-left">{t_vp("spectralCentroidTooltip")}</div></div>
                      </div>
                      <p className={`text-[14px] font-bold tabular-nums ${(result.spectral_features.spectral_centroid_hz ?? 0) < 1500 ? "text-amber-400" : "text-emerald-400"}`}>
                        {Math.round(result.spectral_features.spectral_centroid_hz ?? 0)}<span className="text-[10px] text-zinc-500"> Hz</span>
                      </p>
                    </div>
                    <div className="p-3 rounded-md bg-[#09090b] border border-white/[0.05]">
                      <div className="flex items-center gap-1 mb-1">
                        <span className="text-[10px] text-zinc-500">MFCC Stability</span>
                        <div className="relative group/tt cursor-help"><Info size={9} className="text-zinc-500" /><div className="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-52 rounded-md bg-[#1a1a1e] border border-white/[0.08] p-3 text-[11px] text-zinc-400 leading-relaxed opacity-0 group-hover/tt:opacity-100 transition-opacity z-50 shadow-xl text-left">{t_vp("mfccStabilityTooltip")}</div></div>
                      </div>
                      <p className={`text-[14px] font-bold tabular-nums ${(result.spectral_features.mfcc_stability_score ?? 0) >= 50 ? "text-emerald-400" : "text-amber-400"}`}>
                        {(result.spectral_features.mfcc_stability_score ?? 0).toFixed(1)}<span className="text-[10px] text-zinc-500">/100</span>
                      </p>
                    </div>
                  </div>
                )}
                {result.pitch_contour && (
                  <div className="flex items-center gap-3 p-3 rounded-md bg-[#09090b] border border-white/[0.05]">
                    <div className="flex items-center gap-1 w-24 shrink-0">
                      ...
                      <div className="relative group/tt cursor-help"><Info size={9} className="text-zinc-500" /><div className="pointer-events-none absolute bottom-full left-0 mb-2 w-52 rounded-md bg-[#1a1a1e] border border-white/[0.08] p-3 text-[11px] text-zinc-400 leading-relaxed opacity-0 group-hover/tt:opacity-100 transition-opacity z-50 shadow-xl">{t_vp("pitchContourTooltip")}</div></div>
                    </div>
                    ...
                    <span className="text-[11px] text-zinc-500 ml-auto tabular-nums">{(result.pitch_contour.pitch_slope ?? 0).toFixed(2)} st/s</span>
                  </div>
                )}
                  ...
                  <div className="p-3 rounded-md bg-amber-500/[0.04] border border-amber-500/10">
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-1">
                        ...
                        <div className="relative group/tt cursor-help"><Info size={9} className="text-zinc-500" /><div className="pointer-events-none absolute bottom-full left-0 mb-2 w-52 rounded-md bg-[#1a1a1e] border border-white/[0.08] p-3 text-[11px] text-zinc-400 leading-relaxed opacity-0 group-hover/tt:opacity-100 transition-opacity z-50 shadow-xl">{t_vp("fillerWordsTooltip")}</div></div>
                      </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      ...
                      <div key={m.label} className="p-3 rounded-md bg-[#09090b] border border-white/[0.05]">
                        <div className="flex items-center gap-1 mb-1">
                          <span className="text-[10px] text-zinc-500">{m.label}</span>
                          <div className="relative group/tt cursor-help"><Info size={9} className="text-zinc-500" /><div className="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-52 rounded-md bg-[#1a1a1e] border border-white/[0.08] p-3 text-[11px] text-zinc-400 leading-relaxed opacity-0 group-hover/tt:opacity-100 transition-opacity z-50 shadow-xl text-left">{m.desc}</div></div>
                        </div>
                        <p className={`text-[15px] font-bold tabular-nums ${m.color(m.value)}`}>
                          {(m.value ?? 0).toFixed(1)}<span className="text-[10px] text-zinc-500">{m.unit}</span>
                        </p>
            ...
            {result ? (
              <div className="p-5 space-y-4">
                ...
                {feedbackItems.length > 0 && (
                  <div>
                    <p className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-2">{t_vp("feedbackBreakdown")}</p>
                ...
                {expertTips.length > 0 && (
                  <div>
                    <p className="text-[10px] font-semibold text-zinc-500 uppercase tracking-wider mb-2">{t_vp("coachingTips")}</p>
                    <div className="space-y-2">
              ...
              <div className="py-10 px-6 flex flex-col items-center">
                ...
                <div className="w-full max-w-[240px] mb-3">
                  ...
                  <div className="flex justify-between mt-1.5">
                    <span className="text-[10px] text-zinc-500">{t_vp("aiProcessing")}</span>
                  ...
                  {ANALYZE_PHASES.map((p, i) => {
                    ...
                    return (
                      <div key={i} className="flex items-center gap-2">
                        ...
                        <span className={`text-[11px] transition-colors duration-300 ${done ? "text-emerald-500/70" : active ? "text-zinc-300" : "text-zinc-500"}`}>{p.label}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="py-12 text-center px-5">
                ...
                <p className="text-[11px] text-zinc-500 leading-relaxed max-w-[180px] mx-auto">{t_vp("recordToUnlock")}</p>
              </div>
            )}
          </div>
          ...
          <div className="rounded-md border border-white/[0.07] bg-[#111113] p-5">
            <div className="flex items-center justify-between mb-5">
              ...
              <span className={`text-[11px] font-medium px-2 py-0.5 rounded-md ${result ? "bg-emerald-500/10 text-emerald-400" : "bg-white/[0.04] text-zinc-500"}`}>
            ...
            {result ? (
              <div className="space-y-4">
                <div className="p-4 rounded-md bg-[#09090b] border border-white/[0.06]">
                  <div className="flex items-end justify-between mb-3">
                    <div>
                      <p className="text-[11px] text-zinc-500 mb-1">{t("overallScore")}</p>
                      <div className="flex items-end gap-3">
                        ...
                        {history.length >= 1 && (() => {
                          ...
                          if (Math.abs(delta) < 0.1) return <span className="text-[11px] text-zinc-500 mb-1">{t_vp("sameAsLastTime")}</span>;
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-4 rounded-md bg-[#09090b] border border-white/[0.06]">
                    <div className="flex items-center gap-1.5 mb-2"><BarChart3 size={13} className="text-cyan-400" /><p className="text-[10px] text-zinc-500 uppercase tracking-wider">{t("coverage")}</p></div>
                    <p className="text-2xl font-bold text-white">{completionPercent.toFixed(0)}<span className="text-sm text-zinc-500">%</span></p>
                  </div>
                  <div className="p-4 rounded-md bg-[#09090b] border border-white/[0.06]">
                    <div className="flex items-center gap-1.5 mb-2"><Clock size={13} className="text-purple-400" /><p className="text-[10px] text-zinc-500 uppercase tracking-wider">Pace</p></div>
              ...
              <div className="py-8 text-center">
                ...
                <p className="text-[13px] text-zinc-500">{t("recordToUnlock")}</p>
              </div>
            )}
          </div>
          </div>
