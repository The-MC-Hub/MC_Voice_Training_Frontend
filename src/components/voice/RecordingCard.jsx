...
export default function RecordingCard({
  ...
  return (
    <div data-quest="quest-recording-card" className="flex-1 rounded-md border border-white/[0.07] bg-[#111113] overflow-hidden">
      <div className="rounded-md border-0 overflow-hidden h-full flex flex-col">
        {/* Status bar */}
        ...
        <AnimatePresence>
          {cameraOn && (
            <motion.div
              ...
              style={{ bottom: "1.5rem", right: "1.5rem", width: "220px", aspectRatio: "4/3", borderRadius: "6px", border: "1px solid rgba(255,255,255,0.12)", boxShadow: "0 8px 32px rgba(0,0,0,0.6)", background: "#000" }}
            >
            ...
            {recording && (
              <motion.div key="big-stop" initial={{ opacity: 0, scale: 0.88, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.88, y: 10 }} transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }} className="mt-2 mb-1 flex items-center gap-3">
                <motion.button onClick={stopRecording}
                  ...
                  className="flex items-center gap-2.5 px-8 py-4 rounded-md bg-red-500 text-white text-[15px] font-bold"
            ...
            {audioBlob && !analyzing && !recording && (
              <motion.div key="big-analyze" initial={{ opacity: 0, scale: 0.88, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.88, y: 10 }} transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }} className="mt-2 mb-1 flex items-center gap-3">
                <motion.button onClick={handleAnalyze}
                  ...
                  whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.96 }}
                  className="flex items-center gap-2.5 px-8 py-4 rounded-md bg-gold text-black text-[15px] font-bold"
                  style={{ fontFamily: "'Be Vietnam Pro', sans-serif" }}
                >
                  <Zap size={18} /> {t_vp("analyzeNow")}
                </motion.button>
                <motion.button onClick={resetPractice} whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}
                  className="flex items-center gap-2 px-5 py-4 rounded-md border border-white/12 text-zinc-300 text-[14px] font-semibold hover:bg-white/6 transition-colors"
                  style={{ fontFamily: "'Be Vietnam Pro', sans-serif" }}
                >
          ...
          <AnimatePresence>
            {!recording && !audioBlob && !analyzing && (
              <motion.button key="big-start" onClick={startRecording}
                ...
                whileTap={{ scale: 0.96 }}
                className="relative mt-2 mb-1 flex items-center gap-3 px-12 py-4 rounded-md bg-[#f5a623] text-black text-[15px] font-bold tracking-tight overflow-hidden"
                style={{ fontFamily: "'Be Vietnam Pro', sans-serif" }}
              >
                {[0, 1].map((i) => (
                  <motion.span key={i} className="absolute inset-0 rounded-md border-2 border-gold"
            ...
            {recording && (
              <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 6 }} transition={{ duration: 0.25 }} className="w-full max-w-[280px] px-4">
                {audioStatus === "too_quiet" && (
                  <div className="flex items-center gap-2 px-3 py-2 rounded-md bg-amber-500/[0.08] border border-amber-500/20 text-amber-400 text-[11px]">
                    <Volume1 size={13} className="shrink-0" /><span>{t_vp("audioTooQuiet")}</span>
                  </div>
                )}
                {audioStatus === "too_loud" && (
                  <div className="flex items-center gap-2 px-3 py-2 rounded-md bg-red-500/[0.08] border border-red-500/20 text-red-400 text-[11px]">
                    <Volume2 size={13} className="shrink-0" /><span>{t_vp("audioTooLoud")}</span>
                  </div>
                )}
                {audioStatus === "noisy" && (
                  <div className="flex items-center gap-2 px-3 py-2 rounded-md bg-orange-500/[0.08] border border-orange-500/20 text-orange-400 text-[11px]">
                    <AlertTriangle size={13} className="shrink-0" /><span>{t_vp("audioNoisy")}</span>
                  </div>
                )}
                {audioStatus === "good" && (
                  <div className="flex items-center gap-2 px-3 py-2 rounded-md bg-emerald-500/[0.08] border border-emerald-500/20 text-emerald-400 text-[11px]">
