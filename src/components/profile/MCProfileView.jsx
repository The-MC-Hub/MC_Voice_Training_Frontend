...
const MCProfileView = ({ mc, calendarEvents = [], reviewsList = [], authUser, bookTargetId, isPreview = false }) => {
  ...
  return (
    <div className={`space-y-8 ${isPreview ? '' : 'pb-20'}`}>
        ...
        <style
          dangerouslySetInnerHTML={{
            __html: `
              ...
              .rbc-month-view, .rbc-time-view, .rbc-agenda-view { border: 1px solid rgba(255,255,255,0.08); border-radius: 6px; overflow: hidden; background: rgba(15,23,42,0.45); }
      ...
      <section className={`glass-card relative overflow-hidden rounded-md border border-gold/15 shadow-2xl ${isPreview ? 'p-6' : 'p-8 md:p-10'}`}>
        <div className="absolute inset-0 bg-[rgba(0,35,102,0.95)]" />
        <div className={`relative grid gap-8 ${isPreview ? 'grid-cols-1' : 'lg:grid-cols-[1.3fr_0.8fr]'}`}>
          <div className="space-y-6">
            <div className={`flex flex-col gap-6 ${isPreview ? '' : 'md:flex-row md:items-end'}`}>
              <div className={`relative overflow-hidden rounded-md border border-white/10 shadow-2xl ${isPreview ? 'h-32 w-32' : 'h-40 w-40'}`}>
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-3 text-sm text-slate-200">
                  <div className="flex items-center gap-2 rounded-md border border-white/10 bg-white/5 px-4 py-2">
          ...
          <div className="space-y-4 rounded-md border border-white/10 bg-slate-950/35 p-6 backdrop-blur-xl h-fit">
            ...
            {!isPreview && (
              <>
                {(authUser?.role || "").toLowerCase() === "client" && (
                  <Link to={`/m/booking/${bookTargetId}`} className="btn btn-primary lg w-full rounded-md">
                    Book This MC <ArrowRight size={18} />
                  </Link>
                )}
                <Link to={`/m/messaging?mcId=${bookTargetId}`} className="btn btn-outline w-full rounded-md py-3.5">
      ...
      <div className={`grid gap-8 ${isPreview ? 'grid-cols-1' : 'lg:grid-cols-[1.35fr_0.95fr]'}`}>
        <div className="space-y-8">
          ...
          {mc.eventPhotos && mc.eventPhotos.length > 0 && (
            <section className="glass-card rounded-md border border-white/5 p-8 relative overflow-hidden group/portfolio">
              ...
              <div className="mb-6 flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button 
                    ...
                    className="p-2 rounded-md bg-white/5 border border-white/10 hover:bg-gold/20 hover:border-gold/30 text-white transition-all"
                  >
                    <ChevronLeft size={20} />
                  </button>
                  <button 
              ...
              >
                {mc.eventPhotos.map((photo, index) => (
                  <div 
                    key={index} 
                    className="relative flex-shrink-0 w-72 md:w-80 aspect-[4/5] rounded-md overflow-hidden border border-white/10 snap-start group/photo"
                  >
                    <img 
                      src={photo} 
                      alt={`Event ${index}`} 
                    ...
                    />
                    <div className="absolute inset-0 bg-slate-950/80 opacity-0 group-hover/photo:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-6">
          ...
          <section className="glass-card rounded-md border border-white/5 p-8">
        ...
        {!isPreview && (
          <div className="space-y-8">
            <section className="glass-card rounded-md border border-gold/10 p-8">
              <div className="mb-6 flex items-center gap-3">
              ...
              <div className="space-y-4">
                {mc.packages && mc.packages.map((pkg) => (
                  <div key={`${pkg.name}-${pkg.price}`} className="rounded-md border border-white/8 bg-slate-950/40 p-5">
      ...
      {!isPreview && (
        <section className="glass-card rounded-md border border-white/5 p-8 mt-8">
          ...
          <div className="grid gap-8 md:grid-cols-2">
            {reviewsList.length === 0 ? (
              <div className="col-span-full py-20 text-center border-2 border-dashed border-white/5 rounded-md bg-white/[0.02] backdrop-blur-sm">
              ...
              reviewsList.map((review) => (
                <div 
                  ...
                  className="group relative rounded-md border border-white/10 bg-slate-950/40 p-8 transition-all duration-500 hover:border-gold/40 hover:bg-slate-900/60 hover:shadow-[0_20px_50px_-20px_rgba(212,175,55,0.15)] flex flex-col"
                >
                  {/* Background Accents */}
                  ...
                  <div className="relative flex items-start justify-between mb-8">
                    <div className="flex items-center gap-4">
                      <div className="relative h-14 w-14 rounded-md border border-white/10 bg-gold/20 p-0.5 overflow-hidden">
                        <img
